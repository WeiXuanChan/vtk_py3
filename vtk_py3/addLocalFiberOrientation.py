...
def addLocalFiberOrientation(ugrid_wall,
                             ...
                             cstrainFactor=1.,
                             activeFiberStrength=1.,
                             activeFiberDelay=0.,
                             fiberDirectionVec=None, 
                             fiberDirectionVec_removeRadial=True, 
    ...
    if (verbose): print ("Computing local fiber orientation... \n endo",fiber_angle_end_input,'\n epi',fiber_angle_epi_input)
    if fiber_angle_end_input is None or fiber_angle_epi_input is None:
        if fiber_angle_end_input is None:
            fiber_angle_all=valueAssignment(fiber_angle_epi_input)
        else:
            fiber_angle_all=valueAssignment(fiber_angle_end_input)
    else:
        fiber_angle_end=valueAssignment(fiber_angle_end_input)
        fiber_angle_epi=valueAssignment(fiber_angle_epi_input)
        fiber_angle_all = None
    if fiberDirectionVec is not None:
        fiberDirectionVec=valueAssignment(fiberDirectionVec)
    cstrainFactor=valueAssignment(cstrainFactor)
    activeFiberStrength=valueAssignment(activeFiberStrength)
    activeFiberDelay=valueAssignment(activeFiberDelay)
    ...
    if (type_of_support == "point"):
        farray_norm_dist_end = ugrid_wall.GetPointData().GetArray("norm_dist_end")
        farray_norm_dist_epi = ugrid_wall.GetPointData().GetArray("norm_dist_epi")
        farray_eRR = ugrid_wall.GetPointData().GetArray("eRR")
        farray_eCC = ugrid_wall.GetPointData().GetArray("eCC")
        farray_eLL = ugrid_wall.GetPointData().GetArray("eLL")
    ...
    farray_cstrainFactor=createFloatArray("cstrain_factor", 3, nb_cells)
    farray_activeFiberStrength=createFloatArray("fiber_active_fiber_strength",3, nb_cells)
    farray_activeFiberDelay=createFloatArray("fiber_active_fiber_delay", 3, nb_cells)
    ...
    for num_cell in range(nb_cells):
        ...
        if fiberDirectionVec is not None:
            eF=fiberDirectionVec(cell_centers[num_cell])
            if fiberDirectionVec_removeRadial:
                eF=eF-eF.dot(eRR)*eRR
            eF=eF/numpy.linalg.norm(eF)
            if numpy.dot(eF,eCC)<0:
                eF=-eF
            eS = eRR
            eN = numpy.cross(eF, eS)
            eN=eN/numpy.linalg.norm(eN)
            if not(fiberDirectionVec_removeRadial):
                eS = numpy.cross(eN, eF)
                eS=eS/numpy.linalg.norm(eS)
            fiber_angle_in_degrees =numpy.linalg.norm(numpy.cross(eF,eCC))
            if fiber_angle_in_degrees>=1:
                fiber_angle_in_degrees=90.
            else:
                fiber_angle_in_degrees =numpy.arcsin(fiber_angle_in_degrees)*180./numpy.pi
            if numpy.dot(eF,eLL)<0:
                fiber_angle_in_degrees=-fiber_angle_in_degrees
            farray_fiber_angle.InsertTuple(num_cell, [fiber_angle_in_degrees])
            if not(fiberDirectionVec_removeRadial):
                raise Exception("fiberDirectionVec withput removeRadial is not implemented")
            else:
                ...
                fdensity=1.
        elif fiberSheetletAngle==0. and radialFiberAngle==0.:
            if fiber_angle_all is None:
                ...
                fiber_angle_in_degrees =fiber_angle_all(cell_centers[num_cell])
            farray_fiber_angle.InsertTuple(num_cell, [fiber_angle_in_degrees])
        ...
        farray_cstrainFactor.InsertTuple(num_cell, [cstrainFactor(cell_centers[num_cell]),cstrainFactor(cell_centers[num_cell]),cstrainFactor(cell_centers[num_cell])])
        farray_activeFiberStrength.InsertTuple(num_cell, [activeFiberStrength(cell_centers[num_cell]),activeFiberStrength(cell_centers[num_cell]),activeFiberStrength(cell_centers[num_cell])])
        farray_activeFiberDelay.InsertTuple(num_cell, [activeFiberDelay(cell_centers[num_cell]),activeFiberDelay(cell_centers[num_cell]),activeFiberDelay(cell_centers[num_cell])])
        # farray_cstrainFactor.InsertTuple(num_cell, [cstrainFactor(cell_centers[num_cell])])
        # farray_activeFiberStrength.InsertTuple(num_cell, [activeFiberStrength(cell_centers[num_cell])])
        # farray_activeFiberDelay.InsertTuple(num_cell, [activeFiberDelay(cell_centers[num_cell])])
    ...
    if (type_of_support == "cell"):
        ...
        ugrid_wall.GetCellData().AddArray(farray_cstrainFactor)
        ugrid_wall.GetCellData().AddArray(farray_activeFiberStrength)
        ugrid_wall.GetCellData().AddArray(farray_activeFiberDelay)
    elif (type_of_support == "point"):
        ugrid_wall.GetPointData().AddArray(farray_fiber_angle)
        ugrid_wall.GetPointData().AddArray(farray_fiber_radial_angle)
        ugrid_wall.GetPointData().AddArray(farray_radialPos)
        ugrid_wall.GetPointData().AddArray(farray_sheetlet_angle)
        ugrid_wall.GetPointData().AddArray(farray_sheeletPos)
        ugrid_wall.GetPointData().AddArray(farray_fiber_shift_towards_endo)
        ugrid_wall.GetPointData().AddArray(farray_fiber_normDensity)
        ugrid_wall.GetPointData().AddArray(farray_eF)
        ugrid_wall.GetPointData().AddArray(farray_eS)
        ugrid_wall.GetPointData().AddArray(farray_eN)
        ugrid_wall.GetPointData().AddArray(farray_cstrainFactor)
        ugrid_wall.GetPointData().AddArray(farray_activeFiberStrength)
        ugrid_wall.GetPointData().AddArray(farray_activeFiberDelay)
