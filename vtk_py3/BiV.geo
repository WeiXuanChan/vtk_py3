...
L_epi_base = newll; Curve Loop(L_epi_base) = {Boundary{ Surface {ss[0]}; }};
L_LV_base = newll; Curve Loop(L_LV_base) = {Boundary{ Surface {ss[1]}; }};
L_RV_base = newll; Curve Loop(L_RV_base) = {Boundary{ Surface {ss[2]}; }};

Physical Surface("epi") = {ss[0]};
Physical Surface("LV") = {ss[1]};
Physical Surface("RV") = {ss[2]};

S_base = news; Plane Surface(S_base) = { L_LV_base,L_RV_base,L_epi_base};

Physical Surface("BASE") = {S_base};

SL_wall = newsl; Surface Loop(SL_wall) = {ss[2],ss[0],ss[1] ,S_base};
