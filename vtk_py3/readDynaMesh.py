    ...
    for line in mesh_file:
        ...
        if (context == 'reading nodes'):
            if line.startswith('*'):
                ...
                points.InsertNextPoint([float(coord)
                                        for coord in splitted_line[1:4]])
                if (cell_type == 'vertex'):
            ...
            if line.startswith('*'):
                ...
                if(filetype == 'post'):
                    if (len(splitted_line) == 3 and isfiber == True):
                        fiber_array.InsertNextTuple3(float(splitted_line[0]), float(
                            splitted_line[1]), float(splitted_line[2]))
                        isfiber = False
                        issheet = True
                    elif (len(splitted_line) == 3 and issheet == True):
                        sheet_array.InsertNextTuple3(float(splitted_line[0]), float(
                            splitted_line[1]), float(splitted_line[2]))
                        isfiber = False
                        issheet = False
                    ...
                    if (len(splitted_line) != 3):
                        ...
                        for num_node in range(8):
                            cell.GetPointIds().SetId(
                                num_node, int(splitted_line[2+num_node])-1)
                        isfiber = True
                        cell_array.InsertNextCell(cell)
