...
def convertQuadDataToVTK(mesh, Fspace, data, filename=[]):
	...
	if(not isinstance(data, list)):
		...
		for x_dof, y_dof, z_dof, coord_dof in zip(x_dofs, y_dofs, z_dofs, coord_dofs):
			...
			#norm = math.sqrt(data.vector().get_local()[dofmap[0][p]]**2 +  data.vector().get_local()[dofmap[1][p]]**2 + data.vector().get_local()[dofmap[2][p]]**2)
	...
	if(not (not filename)):
		filename_ = filename + str(MPI.rank(MPI.comm_world)) + '.vtp'
		...
		if(MPI.rank(MPI.comm_world) == 0):
			...
			for p in range(0, MPI.size(MPI.comm_world)):
